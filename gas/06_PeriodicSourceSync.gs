 ...
 * Mapping Kolom Sumber Dealer:
 * - No (Dealer ID): Kolom A (Index 0) -> Format: MTR-0001
 * - Nama Dealer: Kolom B (Index 1)
 * - Nama Owner: Kolom C (Index 2)
 * - Cabang: Kolom F (Index 5)
 * - Tanggal Kerjasama: Kolom AB (Index 27) -> Wajib ada!
 * - Productivity: Kolom AQ (Index 42)
 */
function syncDealersAndFacilitiesFromSource() {
  ...
  if (dealerSheet) {
    ...
    for (let i = 1; i < dData.length; i++) {
      ...
      // Tanggal Kerjasama di Kolom AB (Index 27)
      const tglKerjasama = parseDateValue(row[27]);
      
      // Validasi: Dealer HANYA diproses jika memiliki Tanggal Awal Kerjasama
      if (!tglKerjasama) {
        continue;
      }
  ...
  if (facSheet) {
    ...
    if (fData.length > 3) {
      ...
      for (let i = 3; i < fData.length; i++) {
        ...
        // Filter status pengajuan di Kolom S (Index 18): Pencairan / Pengajuan
        const rawStatusAju = String(row[18] || "").trim().toLowerCase();
...
/**
 * 🚀 FUNGSI UTAMA HARIAN
 * Pasang fungsi ini pada Trigger Google Apps Script (misal: Timer Harian Jam 06:00 Pagi)
 */
